  ...
  public function buildForm(array $form, FormStateInterface $form_state, Request $request = NULL) {
    ...
    foreach ($severity_levels as $severity => $description) {
      ...
      $form['emaillog'][$key] = array(
        '#type'           => 'textfield',
        '#title'          => t('Email address for severity %description', array('%description' => Unicode::ucfirst($description))),
        '#default_value'  => $this->configFactory->get('emaillog.settings')->get($key),
        '#description'    => t('The email address to send log entries of severity %description to.', array('%description' => Unicode::ucfirst($description))),
      );
    }
    ...
    $debug_info_settings = $this->configFactory->get('emaillog.settings')->get('emaillog_debug_info');
    $status = array();
